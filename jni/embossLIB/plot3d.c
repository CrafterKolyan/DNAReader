/* $Id: plot3d.c,v 1.3 2007/05/08 09:09:37 rice Exp $
...
/* pmr: not used ... removed static */
void plxyindexlimits(PLINT instart, PLINT inn, 
...
static void
shade_triangle(PLFLT xx0, PLFLT yy0, PLFLT zz0,
	       PLFLT xx1, PLFLT yy1, PLFLT zz1,
	       PLFLT xx2, PLFLT yy2, PLFLT zz2)
{
  int i;
  ...
  x[0] = xx0; x[1] = xx1; x[2] = xx2;
  y[0] = yy0; y[1] = yy1; y[2] = yy2;
  z[0] = zz0; z[1] = zz1; z[2] = zz2;
  n = 3;
	  ...
	  PLINT ixstart, PLINT ixn, PLINT *indexymin, PLINT *indexymax)
{
  ...
  (void) ixstart;			/* pmr: make these used */
  (void) ixn;
  (void) indexymin;
  (void) indexymax;
...
#define NPTS 100
    int np = NPTS;
    PLFLT *myzz = (PLFLT *) malloc(NPTS*sizeof(PLFLT));
	...
	if (cline->npts > np) {
	  np = cline->npts;
	  myzz = (PLFLT *) realloc(zz, np*sizeof(PLFLT));
	}
	for (j=0; j<cline->npts; j++)
	  myzz[j] = plsc->ranmi;
	if (cline->npts > 0) {
	   plcol1((clev->level-fc_minz)/(fc_maxz-fc_minz));
	   plline3(cline->npts, cline->x, cline->y, myzz);
	}
	cline = cline->next;
      }
      while(cline != NULL);
...
#define max3(a,b,c) (MAX((MAX(a,b)),c))
  ...
  if (opt & DRAW_SIDES) { /* the sides look ugly !!! */
    ...
    PLFLT myzscale, myzmin, myzmax;

    plP_grange(&myzscale, &myzmin, &myzmax);
	...
	if (ixstart <= ix && ix < ixn && 
	 ...
	 break;
      ...
      shade_triangle(px[0], py[0], pz[0], px[2], py[2], pz[2], px[0], py[0], myzmin);
      shade_triangle(px[2], py[2], pz[2], px[2], py[2], myzmin,  px[0], py[0], myzmin);
    }
...
/* Check that points in x and in y are strictly increasing */
    ...
    if(ixmin > 0 || ixmax < nx-1 || iymin > 0 || iymax < ny-1) {
	 ...
	 return;
      ...
      _x[0] = xmin;
      _x[_nx-1] = xmax;
      for(ii=1; ii<_nx-1; ii++)
	_x[ii] = x[ixmin+ii];
      _y[0] = ymin;
      _y[_ny-1] = ymax;
      for(ii=1; ii<_ny-1; ii++)
	_y[ii] = y[iymin+ii];

      /* copy the data array so we can interpolate around the edges */
      for(ii=0; ii<_nx; ii++)
	_z[ii] = (PLFLT*)malloc(_ny * sizeof(PLFLT));
      ...
      for(ii=0; ii<_nx; ii++) {
	if(ii==0) {
	  _z[ii][0] = z[ixmin][iymin]*(1-ty0)*(1-tx0) + z[ixmin][iymin+1]*(1-tx0)*ty0
	    + z[ixmin+1][iymin]*tx0*(1-ty0) + z[ixmin+1][iymin+1]*tx0*ty0;
	  for(jj=1; jj<_ny-1; jj++)
	    _z[ii][jj] = z[ixmin][iymin+jj]*(1-tx0) + z[ixmin+1][iymin+jj]*tx0;
	  _z[ii][_ny-1] = z[ixmin][iymax-1]*(1-tx0)*(1-ty1) + z[ixmin][iymax]*(1-tx0)*ty1
	    + z[ixmin+1][iymax-1]*tx0*(1-ty1) + z[ixmin+1][iymax]*tx0*ty1;
	} else if(ii==_nx-1) {
	  _z[ii][0] = z[ixmax-1][iymin]*(1-tx1)*(1-ty0) + z[ixmax-1][iymin+1]*(1-tx1)*ty0
	    + z[ixmax][iymin]*tx1*(1-ty0) + z[ixmax][iymin+1]*tx1*ty0;
	  for(jj=1; jj<_ny-1; jj++)
	    _z[ii][jj] = z[ixmax-1][iymin+jj]*(1-tx1) + z[ixmax][iymin+jj]*tx1;
	  _z[ii][_ny-1] = z[ixmax-1][iymax-1]*(1-tx1)*(1-ty1) + z[ixmax][iymax]*(1-tx1)*ty1
	    + z[ixmax][iymax-1]*tx1*(1-ty1) + z[ixmax][iymax]*tx1*ty1;
	} else {
	  _z[ii][0] = z[ixmin+ii][iymin]*(1-ty0) + z[ixmin+ii][iymin+1]*ty0;
	  for(jj=1; jj<_ny-1; jj++)
	    _z[ii][jj] = z[ixmin+ii][iymin+jj];
	  _z[ii][_ny-1] = z[ixmin+ii][iymax-1]*(1-ty1) + z[ixmin+ii][iymax]*ty1;
	}
	for(jj=0; jj<_ny; jj++) {
	  if(_z[ii][jj] < zmin)
	    _z[ii][jj] = zmin;
	  else if(_z[ii][jj] > zmax)
	    _z[ii][jj] = zmax;
	}
      }
   ...
   (perpendicular to x-y plane) must be handled separately. */
	...
	do { /* there are several lines that make up each contour */
	  ...
	  if (cline->npts > 0) {
	    do {
	      plcol1((clev->level-fc_minz)/(fc_maxz-fc_minz));
	      cx =  plP_wcpcx(plP_w3wcx(cline->x[ii],cline->y[ii], plsc->ranmi));
	      for (jj=ii; jj < cline->npts; jj++) {  /* convert to 2D coordinates */
		uu[jj] = plP_wcpcx(plP_w3wcx(cline->x[jj],cline->y[jj], plsc->ranmi));
		vv[jj] = plP_wcpcy(plP_w3wcy(cline->x[jj],cline->y[jj], plsc->ranmi));
		if (uu[jj] < cx) /* find turn back point */
		  break;
		else
		  cx = uu[jj];
	      }
	      plnxtv(&uu[ii], &vv[ii], NULL, jj-ii, 0); /* plot line with increasing x */
	      
	      if (jj < cline->npts) { /* line not yet finished, */
		start = jj-1;
		for (ii = start; ii < cline->npts; ii++) {  /* search turn forward point */
		  uu[ii] = plP_wcpcx(plP_w3wcx(cline->x[ii],cline->y[ii], plsc->ranmi));
		  if (uu[ii] > cx)
		    break;
		  else
		    cx = uu[ii];
		...
		ii = end; /* restart where it was left */
	      }
	    } while (jj < cline->npts && i < cline->npts);
	  }
	  cline = cline->next;
	}
	while(cline != NULL);
...
void					/* pmr: not used - removed static */
       ...
       PLINT *u, PLINT *v, PLFLT* c)
{
	...
	v[n] = plP_wcpcy(y2d);
	if (c != NULL)
	  c[n] = (z[xx0 - 1][yy0 - 1] - fc_minz)/(fc_maxz-fc_minz);
