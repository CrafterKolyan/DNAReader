...
#define PL_OPT_INT		0x0400	/* Set *var = atoi(myoptarg) */
#define PL_OPT_FLOAT		0x0800	/* Set *var = atof(myoptarg) */
#define PL_OPT_STRING		0x1000	/* Set var = myoptarg */
...
typedef struct {
    const char *opt;			/* pmr: const */
    int  (*handler)	(const char *, const char *, void *);
    void *client_data;
    void *var;
    long mode;
    const char *syntax;			/* pmr: const */
    const char *desc;			/* pmr: const */
} PLOptionTable;
...
/* pmr: moved name to top to fill in alignment */
typedef struct {
...
PLDLLIMPEXP int
c_plsetopt(char *opt, char *myoptarg);

PLDLLIMPEXP int
plSetOpt(char *opt, char *myoptarg);
